...
set tr [open p13_mix.tr w]
$ns trace-all $tr

set qf [open p13_mix_qmon.tr w]
...
proc finish {} {
    ...
    close $tr
    close $qf
    puts "Done. p13_mix.tr , p13_mix_qmon.tr"
    exit 0
}
...
set TCP_A "Agent/TCP/Newreno"
set TCP_B "Agent/TCP/Vegas"
set TCP_C "Agent/TCP/Sack1"

set t1 [make_flow $ns $s0 $d0 1 $TCP_A $SINK 0.5]
set t2 [make_flow $ns $s0 $d0 2 $TCP_A $SINK 0.7]

set t3 [make_flow $ns $s1 $d1 3 $TCP_B $SINK 0.9]
set t4 [make_flow $ns $s1 $d1 4 $TCP_B $SINK 1.1]

set t5 [make_flow $ns $s2 $d2 5 $TCP_C $SINK 1.3]
set t6 [make_flow $ns $s2 $d2 6 $TCP_C $SINK 1.5]
